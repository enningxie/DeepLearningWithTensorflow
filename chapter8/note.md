...
2015年时，谷歌的研究人员使用LSTM减少了49%的语音识别错误，这是一个飞跃性进步。而后，众多语音识别方面的研究都会借助LSTM的概念，语音识别技术也成为人工智能领域商业化最快的技术之一。

LSTM作为RNN的一种变体

LSTM的优势：  
可以解决梯度消失的问题，这一问题是影响RNN表现的重大障碍  
LSTM可以保留误差，用于沿时间和层进行反向传递。LSTM将误差保持在更为恒定的水平，让RNN能够进行多个时间步的学习（超过1000个时间步），从而打开了建立远距离因果联系的通道。  
LSTM将信息存放在递归神经网络正常信息流之外的门控单元中。这些单元可以存储、写入或读取信息，就像计算机内存中的数据一样。单元通过门的开关判定存储哪些信息，以及何时允许读取、写入或清除信息。但与计算机中的数字式存储器不同的是，这些门是模拟的，包含输出范围全部在0~1之间的sigmoid函数的逐元素相乘操作，相比数字式存储，模拟值的优点是可微分，因此适合反向传播。  
...
LSTM能够提升神经网络接受输入信息及训练数据的记忆机制，让输出结果得到大幅度的提升。

RNN（循环神经网络）能够让信息在网络中再次循环的网络  
其实它与传统神经网络类似，只不过是把同一网络多次复制，并将信息传递到下一个网络。  
像链条一样的网络实际上也体现出RNN的一个特点：它与序列或列表有着紧密的联系，而它们是处理此类数据的重要架构。  
更重要的是，它们确实得以顺利应用，在过去的几年时间中，RNN被成功应用于解决各种问题：语音识别、语言建模、机器翻译、等  
成功的关键在于它们使用了LSTM。LSTM是RNN的一种特殊形式，而它的普适性比传统的RNN要好的多。几乎所有激动人心的发现都是在LSTM上实现的。

RNN的一大魅力在于它能够将此前的信息结合到当前的任务中，  
当距离增加时，RNN能够将相关信息串联起来的能力也就越弱  
解决方案LSTM

LSTM网络  
长短期记忆网络——通常也被简称为LSTMs——是一种特殊类型的RNN，能够学习长期的依赖关系。

LSTM就是为了解决长期依赖而生的，也就是说，对信息的长期记忆是它们的自发行为，而不是刻意去学习的。

所有的RNN都具备重复的链条形式，而在标准的RNN中，这个重复模式都有着一个简单的结构，比如单层的tanh层。

---

自然语言建模

语言模型的目的是为了计算一个句子的出现概率。在这里把句子看成是单词的序列，于是语言模型需要计算的就是p(w1,w2,w3,...,wm)。利用语言模型可以确定哪个单词序列出现的可能性更大，或者给定若干单词，可以预测下一个最可能出现的词语。

为了估计这些参数，常见的方法有n-gram方法/决策树、最大熵模型、条件随机场、神经网络语言模型等等，这里我们介绍最简单的n-gram模型来介绍语言模型问题。

n-gram模型里的n指的是当前单词依赖它前面的单词的个数。当n越大时，n-gram模型理论上越准确，但也越复杂，需要的计算量和训练语料数据量也就越大。n通常取1,2,3，取n>=4的情况非常少。n-gram模型的参数一般采用最大似然估计的方法计算。

如果一个语言模型的perplexity是10，就表示，平均情况下，模型预测下一个词时，有10个词等可能地可以作为下一个词的合理选择。

我们这里给出用循环神经网络来对自然语言建模。

所用的数据集：PTB文本数据集是语言模型学习中目前最广泛使用的数据集。

从输出可以看出在迭代开始时，perplexity值为10000，这基本相当于从10000个单词中随机选择下一个单词。而训练结束后，在训练数据上的perplexity值降低到了172.这表明通过训练调整LSTM隐藏层的节点个数和大小以及训练迭代的轮数还可以将perplexity值降到更低。

---

如何在语音识别中使用深度学习方法提高系统性能是语音识别领域的一个重要的研究内容。

---

## 梯度消失问题

目前深度学习面临的一个问题就是在网络训练过程中存在**梯度消失问题**(vanishing gradient problem)，或者更广义的来讲就是**不稳定梯度问题**。

第二层神经元的学习速率大于第一层神经元学习速率

实验表明第一层的学习速度和最后一层要差两个数量级。实际上这个问题是可以避免的，尽管替代方法不是那么有效，同样会产生问题——在前面的层中的梯度会变得非常大！这也叫做激增的梯度问题（exploding gradient problem），这也没有比梯度消失问题更好处理。更一般地说，在深度神经网络中的梯度是不稳定的，在前面的层中或会消失，或会激增，这种不稳定性才是深度神经网络中基于梯度学习的根本原因。

不稳定的梯度问题：根本的问题其实并非是小时的梯度问题或者激增的梯度问题，而是在前面的层上的梯度是来自后面的层上项的乘积。当存在过多的层次时，就出现了内在本质上的不稳定场景。唯一让所有层都接近相同的学习速率的方式是所有这些项的乘积都能得到一种平衡。如果没有某种机制或者更加本质的保证达到平衡，那网络就很容易不稳定了。简而言之，真实的问题就是神经网络受限于不稳定梯度的问题。所以，如果我们使用标准的基于梯度的学习算法，在网络中的不同层会出现按照不同学习速率学习的情况。
